...
| 模块 | 说明 |
| --- | --- |
| 主页 | 数据概览 + **模拟出题**（图画/图表/书信/通知，随机或指定题型）+ 真题抽题 + 快速入口 |
...
**配图能力**：图表作文（英语二全部 + 2025 英语一）内置按公开真题数据复刻的 SVG 图表；图画作文（英语一 2010–2024）内置按真题描述绘制的示意图画；模拟题会自动生成与考试同标准的图表/图画。
...
- 真题配图支持三种形式（在对应真题的 `image` 字段中任选其一）：
  ...
  2. **示意图画**：`{ "scene": { "id": "sc-xxx", "caption": "图画内容描述" } }`；
  3. **本地原图**：`{ "file": "images/en1-2015.png" }` —— 把真题原图放入项目根目录 `images/` 文件夹后引用即可（推荐用于替换成与真题一致的原图）。
...
├── images/                   # （可选）真题原图，供 image.file 引用
├── js/
...
- **配图**：真题图表按公开数据**近似复刻**（数值贴近公开资料）；图画作文为站内绘制的**示意复刻图**，与真题原图在风格上接近但非完全相同（原图版权归考试命题方）。如需与真题完全一致的原图，请将原图放入 `images/` 目录并在对应真题的 `image.file` 中引用。
